...
///
/// Security rules:
/// - `/_va_/*` paths → serve dashboard SPA (never proxy)
...
pub async fn cookie_proxy_fallback(
    ...
    // Never proxy /_va_/ paths — they belong to the dashboard.
    let path = req.uri().path();
    if path.starts_with("/_va_") || path == "/_va_" {
    ...
    let slug = match slug {
        Some(s) => s,
        None => return Redirect::temporary("/_va_/").into_response(),
    };
    ...
    if is_direct_navigation {
        return Redirect::temporary("/_va_/").into_response();
    }
